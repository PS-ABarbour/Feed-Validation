...
def getNaNs(feed,saved_feed):
    ...
    file = pd.read_excel(feed, dtype={'sku': str})
...
# *** NOT A NECESSARY FUNCTION PER USER, NOT ADDED INTO FINAL FUNCTION ***

# Image validation function
# Ensure provided image urls are actually URLs, and not broken. Caveat is some valid urls will get flagged. Validators.url() is not robust, but will work ~75% of the time. Other solutions have very long run times +-20s.
...
def country_code_validation(df):
    ...
    abspathccdf = os.path.abspath(sys.argv[0]) #cds to dir where the actual python script is stored, reads in required .xlsx files and cds back to feed dir.
    dname2 = os.path.dirname(abspathccdf)
    os.chdir(dname2)
    ccdf = pd.read_excel('country_codes.xlsx') 
    os.chdir(file_dir)
...
def lang_code_validation(df):
    ...
    abspathlcdf = os.path.abspath(sys.argv[0]) #cds to dir where the actual python script is stored, reads in required .xlsx files and cds back to feed dir. 
    dname3 = os.path.dirname(abspathlcdf)
    os.chdir(dname3)
    ccdf = pd.read_excel('lang_codes.xlsx')
    os.chdir(file_dir)

   
    dataDict = df.to_dict('records')
    badCountries = []
    errors_xl = pd.read_excel(Feed_Errors_Name)
    errors_df = pd.DataFrame(errors_xl)
...
def productGroup_validation(df, columnName, brandColumnName):
    ...
    for x in dataDict:
        pg = x[columnName]
        country = x["Country"]
        brand = x[brandColumnName].replace(' ','')
...
def General_Feed_Validation(feed,saved_feed):
    ...
    goodfeed2, badfeed2 = upc_validation(goodfeed)
    goodfeed3, badfeed3 = dup_validation(goodfeed2,thirdColumn=None)
    goodfeed4, badfeed4 = leading_zeros(goodfeed3,badfeed3) # function doesn't need do anything to badfeed4, just there to avoid errors. Badfeed5 is ultimately skipped as it is the same as badfeed4.
    goodfeed5, badfeed5 = country_code_validation(goodfeed4)

    dfbadfull = pd.concat([badfeed,badfeed2,badfeed3,badfeed4,badfeed5])
...
def PNG_Feed_Validation(feed,saved_feed):
    ...
    goodfeed, badfeed = getNaNs(feed,saved_feed)
    goodfeed2, badfeed2 = upc_validation(goodfeed)
    goodfeed3, badfeed3 = dup_validation(goodfeed2, thirdColumn='resourceLanguage')
    goodfeed4, badfeed4 = country_code_validation(goodfeed3)
    goodfeed5, badfeed5 = lang_code_validation(goodfeed4)
    goodfeed6, badfeed6 = productGroup_validation(goodfeed5, "productGroup", "manufacturerName")

    dfbadfull = pd.concat([badfeed,badfeed2,badfeed3,badfeed4,badfeed5,badfeed6])
...
# Capturing common errors to provide feedback to user.
        ...
        result['text'] = 'Unsupported file type (i.e. xml) or potentially corrupt file.'
    except FileNotFoundError:
        result['text'] = 'Required files not found in current directory.\nCheck that country_codes.xlsx and lan_codes.xlsx are in same folder as validator.'
...
# progress_frame = Frame(root,bg='#6f7b91')
# progress_frame.place(relx=0.32, rely=0.5,relwidth=0.3,relheight=0.09)

# progress = Progressbar(frame, orient = 'horizontal', length = 100, mode = 'determinate',style='Horizontal.TProgressbar')
# progress.place(relx=0.32, rely=0.5,relwidth=0.2,relheight=0.09)

# blank = Frame(progress_frame,bg='#6f7b91')
# blank.place(relx=0.32, rely=0.5,relwidth=0.3,relheight=0.09)
